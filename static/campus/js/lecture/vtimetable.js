...
var Lecture = (function () {
    ...
    Lecture.prototype.get_data = function (name) {
    ...
    for (var i = 0; i < lecture_view_list.length; i++) {
        ...
        for (var j = 0; j < lecture_time.length; j++) {
            ...
            if (time_info['weekday'] == 0)
                weekday = 'mon';
            else if (time_info['weekday'] == 1)
                weekday = 'tue';
            else if (time_info['weekday'] == 2)
                weekday = 'wed';
            else if (time_info['weekday'] == 3)
                weekday = 'thu';
            else if (time_info['weekday'] == 4)
                weekday = 'fri';
            else if (time_info['weekday'] == 5)
                weekday = 'sat';
            ...
            $('#' + weekday + '_' + (time_info.start + 1)).html('<div class="lecture-timetable-overlay"><a href="javascript:void(0)" onclick="lecture_view_onclick(\'' + lecture.get_data('code') + '\')" onmouseover="lecture_onmouseover(\'' + lecture.get_data('code') + '\')" onmouseout="lecture_onmouseout(\'' + lecture.get_data('code') + '\')" >' + lecture.get_data('subject_name') + '<br>' + lecture.get_data('code') + '</a></div>');
        }
    }
    if (mouseover_lecture != null) {
        for (var i = 0; i < lecture_view_list.length; i++) {
            if (lecture_view_list[i].get_data('code') == mouseover_lecture.get_data('code'))
                return;
        }
        ...
        for (var arr_i = 0; arr_i < arr.length; arr_i++) {
            ...
            for (var i = 0; i < arr[arr_i].get_data('timetable').length; i++) {
                ...
                for (var timetable_i = time.start; timetable_i < time.end; timetable_i++) {
                    if (timetable[time.weekday][timetable_i])
                        return;
...
$(function () {
    ...
    for (var time = 1; time <= 24; time++) {
        ...
        $('.timetable').append('<div class="row"> \
                <div class="cell-time"> \
                    <p>' + time_string + '</p> \
                </div> \
